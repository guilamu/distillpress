...
class DistillPress_GitHub_Updater
{
	 ...
	 * @var string
	 */
	private const PLUGIN_DESCRIPTION = 'AI-powered article summarization and automatic category selection using POE or Google Gemini API. Distill your content to its essence.';
	...
	private static function parse_readme(): array
	{
		...
		for ($i = 1, $count = count($parts); $i < $count; $i++) {
			...
			if ('installation' === $title) {
				...
				// Include in description (e.g. feature sections).
				$description .= "\n\n## " . trim($lines[0]) . "\n" . $body;
			}
		}
