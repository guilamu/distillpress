...
define( 'DISTILLPRESS_VERSION', '1.0.0' );
...
final class DistillPress {
	...
	public function enqueue_admin_assets( $hook ) {
		...
		$localize_data = array(
			...
			'i18n'    => array(
				...
				'generating_summary'   => __( 'Generating summary and teaser...', 'distillpress' ),
				...
				'summary_generated'    => __( 'Summary and teaser generated successfully!', 'distillpress' ),
	...
	public function ajax_generate_summary() {
		...
		$num_points = min( max( $num_points, 1 ), 20 ); // Enforce 1-20 range.
		$reduction_percent = isset( $_POST['reduction_percent'] ) ? absint( $_POST['reduction_percent'] ) : 0;
		$reduction_percent = min( $reduction_percent, 100 ); // Enforce max 100.
		...
		// Build the optimized prompt for factual, source-based summarization and teaser
		$system_prompt = __( 'You are a precise summarization assistant. Your task is to create factual summaries and teasers based EXCLUSIVELY on the provided text. You must:', 'distillpress' ) . "\n\n" .
			...
			__( '4. Use neutral, objective language for the summary', 'distillpress' ) . "\n" .
			__( '5. Make the teaser engaging but still factual', 'distillpress' ) . "\n" .
			__( '6. Preserve the original meaning accurately', 'distillpress' ) . "\n" .
			__( '7. Respond in the SAME LANGUAGE as the source text', 'distillpress' ) . "\n" .
			__( '8. Return your response in JSON format with "summary" and "teaser" fields', 'distillpress' );

		/* translators: %d: number of bullet points */
		$user_prompt = sprintf(
			__( 'Analyze the following article and provide both a summary and a teaser.', 'distillpress' ),
			$num_points
		) . "\n\n";
		...
		$result = DistillPress_POE_API_Service::chat_with_system(
			...
			$system_prompt,
			$user_prompt,
			0.4, // Slightly higher for engaging teaser while staying factual
			2500
		);
		...
		if ( ! is_array( $parsed ) || ! isset( $parsed['summary'] ) || ! isset( $parsed['teaser'] ) ) {
